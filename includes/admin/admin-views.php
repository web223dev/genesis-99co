...
add_theme_support( 'admin-bar', array('callback' => '__return_false') );
	...
	add_action( 'admin_bar_init', 'nnco_remove_admin_bar_inline_css' );
	function nnco_remove_admin_bar_inline_css () {
 ...
 */
	...
	add_action( 'admin_bar_menu', 'nnco_hide_admin_bar_avatar', 0 );
	function nnco_hide_admin_bar_avatar () {

	add_filter( 'pre_option_show_avatars', '__return_zero' );

}

	add_action( 'admin_bar_menu', 'nnco_restore_avatars', 10 );
	function nnco_restore_avatars () {
 ...
 * @since 2.0.0
 */
	add_filter( 'show_admin_bar', 'nnco_maybe_hide_admin_bar', 99 );
	function nnco_maybe_hide_admin_bar ( $default ) {

	return current_user_can( 'edit_posts' ) ? $default : false;

}

	add_action( 'admin_menu', 'nnco_remove_dashboard_widgets' );
	/**
	 ...
	 * @since 1.x
	 */
	function nnco_remove_dashboard_widgets () {
	...
	add_action( 'widgets_init', 'nnco_unregister_default_widgets' );
	/**
	 * Disable some or all of the default widgets.
	 *
	 * @since 2.0.0
	 */
	function nnco_unregister_default_widgets () {
	...
	add_filter( 'default_hidden_meta_boxes', 'nnco_hidden_meta_boxes', 2 );
	/**
	 * Change which meta boxes are hidden by default on the post and page edit screens.
	 *
	 * @since 2.0.0
	 */
	function nnco_hidden_meta_boxes ( $hidden ) {
	...
	// add_action( 'admin_footer-post-new.php', 'nnco_media_manager_default_view' );
	// add_action( 'admin_footer-post.php', 'nnco_media_manager_default_view' );
	/**
	 ...
	 * @since 2.0.11
	 */
	function nnco_media_manager_default_view () {
	...
	// add_filter( 'posts_where', 'nnco_restrict_attachment_viewing' );
	/**
	 ...
	 * @since 2.0.20
	 */
	function nnco_restrict_attachment_viewing ( $where ) {
	...
	// add_action( 'admin_init', 'nnco_add_editor_style' );
/*
 * Add a stylesheet for TinyMCE
 *
 * @since 2.0.0
 */
	function nnco_add_editor_style () {
	...
	// add_filter( 'mce_external_plugins', 'nnco_add_tinymce_plugins' );
/*
 * Add a plugin script for TinyMCE
 *
 * @since 2.3.35
 */
	function nnco_add_tinymce_plugins ( $plugin_array ) {

		$use_production_assets = genesis_get_option( 'nnco_production_on' );
	$use_production_assets  = !empty($use_production_assets);

		$assets_version    = genesis_get_option( 'nnco_assets_version' );
	...
	$src = add_query_arg(
		...
		$plugin_array[ 'nnco_admin' ] = get_stylesheet_directory_uri() . $src;

	return $plugin_array;

}

	add_filter( 'tiny_mce_before_init', 'nnco_tiny_mce_before_init' );
	/**
	 ...
	 * @since 2.0.0
	 */
	function nnco_tiny_mce_before_init ( $options ) {
	...
	add_filter( 'mce_buttons', 'nnco_tinymce_buttons' );
	/**
	 ...
	 * @since 2.0.15
	 */
	function nnco_tinymce_buttons ( $buttons ) {
	...
	add_filter( 'user_contactmethods', 'nnco_user_contactmethods' );
	/**
	 ...
	 * @since 2.0.0
	 */
	function nnco_user_contactmethods ( $fields ) {
	...
	// add_action( 'admin_menu', 'nnco_remove_dashboard_menus' );
	/**
	 * Remove default admin dashboard menus.
	 *
	 * @since 2.0.0
	 */
	function nnco_remove_dashboard_menus () {
	...
	add_filter( 'login_errors', 'nnco_login_errors' );
	/**
	 ...
	 * @since 2.0.0
	 */
	function nnco_login_errors () {

	return __( 'Invalid username or password.', CHILD_THEME_TEXT_DOMAIN );

}

	add_action( 'admin_head', 'nnco_hide_admin_help_button' );
	/**
	 ...
	 * @since 2.0.0
	 */
	function nnco_hide_admin_help_button () {
	 ...
	 * @since 2.2.8
	 */
	// add_filter( 'theme_page_templates', 'nnco_deregister_page_templates' );
	function nnco_deregister_page_templates ( $templates ) {
	...
	add_action( 'admin_bar_menu', 'nnco_admin_menu_plugins_node' );
	/**
	 * Add a plugins link to the appearance admin bar menu.
	 *
	 * @since 2.2.9
	 */
	function nnco_admin_menu_plugins_node ( $wp_admin_bar ) {
